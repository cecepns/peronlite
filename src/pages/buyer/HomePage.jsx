  ...
  return (
    <div className="-mx-3 min-h-full min-w-0 bg-white px-3 pb-6 sm:-mx-4 sm:px-4">
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex min-w-0 flex-1 items-center justify-end gap-1 truncate text-sm font-medium text-slate-500 sm:justify-center"
            >
      ...
      <div className="min-w-0 space-y-3">
        ...
        />
        <section
          data-intro-home-banner
          className="w-full min-w-0 max-w-full shrink-0"
          aria-label="Banner promosi"
        >
          {loading ? (
            <div
              className={`w-full max-w-full animate-pulse rounded-2xl border border-slate-200 bg-white ${BANNER_HEIGHT_CLASS}`}
            />
          ) : banners.length > 0 ? (
        ...
        <Link
          to={
            storeCategory
              ? `/roof-top?store_category=${encodeURIComponent(storeCategory)}`
              : "/roof-top"
          }
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-[10px] font-bold uppercase tracking-widest text-slate-800/70">
              Marketplace Hasil Bumi
            </span>
            <span className="block text-lg font-extrabold leading-tight text-slate-900 sm:text-xl">
              Roof Top — Komoditas Pertanian
            </span>
            <span className="mt-0.5 block text-xs font-medium text-slate-800/80">
              Lihat daftar penjual & harga komoditas per daerah
            </span>
          </span>
          <ChevronRight
            size={22}
            className="shrink-0 text-slate-900 transition group-hover:translate-x-0.5"
          />
        </Link>
      ...
      <RegencySearchModal
        ...
        onSelect={({ code, name, province_name }) => {
          setRegencyCode(code || "");
          setRegencyLabel(
            !code ? "" : province_name ? `${name} · ${province_name}` : name,
          );
        }}
      />
      ...
      >
        ...
        <p className="mb-4 text-xs text-slate-500">
          Semua kab/kota Indonesia — ketik untuk mencari di daftar.
        </p>
        <button
          type="button"
          onClick={() => {
